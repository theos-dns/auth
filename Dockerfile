...
CMD ["/bin/sh", "-c", "./nginx-forward-generator -to ${FORWARD_TO:?} -port ${PORTS:?} -protect ${PROTECT:-''} -resolver ${RESOLVER:-127.0.0.53:53} -nginx-conf-file $nginxConfFile -allowed-ips-file $allowedIpsFile && sleep ${STARTUP_SLEEP:-0} && nginx -g 'daemon off;' | /root/app/auth/auth -db ${DB_PATH:?} -allowed-ips-file $allowedIpsFile -upstream ${UPSTREAM:-''}"]
